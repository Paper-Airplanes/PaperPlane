...
public:
	...
	Wave(float waveWidth, float waveHeight, 
		...
		string texturePath);
		*/
	Wave(float waveWidth, float waveHeight, string texturePath);
	~Wave();
	void prepare() {
		//setTexture();
	}
...
Wave::Wave(float _waveWidth, float _waveHeight, 
	...
	*/
	...
	createVBO_VAO("grid");
	prepare();
	//waveProgram = new Shader(gridVertexShader.c_str(), gridFragShader.c_str());
	//setUniform();
	//setTexture();
}
...
/*
void Wave::setUniform(Shader* shader) {
	shader->use();
